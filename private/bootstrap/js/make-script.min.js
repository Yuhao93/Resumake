...
function revalidateEducationField(){$("#education-container").html("");for(var a=0;a<resume.educationInfo.length;a++){for(var b=resume.educationInfo[a].startDate,c=resume.educationInfo[a].endDate,d=resume.educationInfo[a].awards,b='<div class="education-object" id = "edu-obj-'+a+'"><a class="close" id="education-delete-'+a+'">X</a><h3><a class="education-edit-link" id="education-edit-'+a+'" href="#education-modal" data-toggle="modal">'+resume.educationInfo[a].school+"</a></h3><p><strong>"+resume.educationInfo[a].degree+
"</strong> "+getFormattedDate(b)+" - "+(""==getFormattedDate(c)&&""!=getFormattedDate(b)?"Ongoing":""==getFormattedDate(c)&&""!=getFormattedDate(b))?"Ongoing":""==getFormattedDate(c)&&""!=getFormattedDate(b)?"Ongoing":getFormattedDate(c)+"</p>",c=0;c<d.length;c++)b+="<p><strong>"+d[c]+"</strong></p>";b+="</div>";$("#education-container").append(b);$("#education-edit-"+a).click(function(){var a=$(this).attr("id").substring(15);editEducation(a)});$("#education-delete-"+a).click(function(){var a=$(this).attr("id").substring(17);
resume.educationInfo.splice(a,1);$("#edu-obj-"+a).remove()})}}
...
function revalidateExperienceField(){$("#experience-container").html("");for(var a=0;a<resume.experienceInfo.length;a++){for(var b=resume.experienceInfo[a].startDate,c=resume.experienceInfo[a].endDate,d=resume.experienceInfo[a].items,b='<div class="experience-object" id = "experience-object-'+a+'"><a class="close" id="experience-delete-'+a+'">X</a><h3><a class="experience-edit-link" id="experience-edit-'+a+'" href="#experience-modal" data-toggle="modal">'+resume.experienceInfo[a].position+"</a></h3><h3>"+
resume.experienceInfo[a].group+" "+getFormattedDate(b)+" - "+(""==getFormattedDate(c)&&""!=getFormattedDate(b))?"Ongoing":getFormattedDate(c)+"</h3>",c=0;c<d.length;c++)"link"==d[c].type?b+='<a href="'+d[c].link+'" target="_blank">'+d[c].name+"</a>":"desc"==d[c].type&&(b+="<p><strong>"+d[c].desc+"</strong></p>");b+="</div>";$("#experience-container").append(b);$("#experience-edit-"+a).click(function(){var a=$(this).attr("id").substring(16);editExperience(a)});$("#experience-delete-"+a).click(function(){var a=
$(this).attr("id").substring(18);resume.experienceInfo.splice(a,1);revalidateExperienceField()})}}
...
function revalidateActivityField(){$("#activity-container").html("");for(var a=0;a<resume.activityInfo.length;a++){for(var b=resume.activityInfo[a].startDate,c=resume.activityInfo[a].endDate,d=resume.activityInfo[a].items,b='<div class="activity-object" id = "activity-object-'+a+'"><a class="close" id="activity-delete-'+a+'">X</a><h3><a class="activity-edit-link" id="activity-edit-'+a+'" href="#activity-modal" data-toggle="modal">'+resume.activityInfo[a].position+"</a></h3><h3>"+resume.activityInfo[a].group+
" "+getFormattedDate(b)+" - "+(""==getFormattedDate(c)&&""!=getFormattedDate(b))?"Ongoing":getFormattedDate(c)+"</h3>",c=0;c<d.length;c++)"link"==d[c].type?b+='<a href="'+d[c].link+'" target="_blank">'+d[c].name+"</a>":"desc"==d[c].type&&(b+="<p><strong>"+d[c].desc+"</strong></p>");b+="</div>";$("#activity-container").append(b);$("#activity-edit-"+a).click(function(){var a=$(this).attr("id").substring(16);editActivity(a)});$("#activity-delete-"+a).click(function(){var a=$(this).attr("id").substring(18);
resume.activityInfo.splice(a,1);revalidateActivityField()})}}function editActivity(a){activityEdit.isEdit=!0;activityEdit.index=a;activityEdit.items=resume.activityInfo[a].items;var b=resume.activityInfo[a].position,c=resume.activityInfo[a].startDate,d=resume.activityInfo[a].endDate,a=resume.activityInfo[a].group;$("#activity-position").attr("value",b);$("#activity-start-date").attr("value",c);$("#activity-end-date").attr("value",d);$("#activity-group").attr("value",a);repopulateActivityItems()}
function getFormattedDate(a){if(void 0==a||-1==a.indexOf("-"))return"";var b=a.split("-")[0],a=a.split("-")[1];"0"==a.charAt(0)&&(a=a.substring(1));return"January February March April May June July August September October November December".split(" ")[parseInt(a)-1]+" "+b}
...
!1;activityEdit.items=[]});$("#code-preview").click(function(){completeResume()});$("#code-submit").click(function(){completeResume();var a=$("#basic-resume").attr("value");alert(a);$.post("../private/php_scripts/addResume.php",{uid:uid,resume:resume,name:a},function(a){window.location.href="../rmks/"+username+"/"+a})})});
