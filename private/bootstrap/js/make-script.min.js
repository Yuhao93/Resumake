...
function addLinkExperience(){var a=experienceEdit.items.length,b=$("#experience-link-name").attr("value"),c=$("#experience-link").attr("value");0!=c.indexOf("http")&&(c="http://"+c);$("#experience-link-name").attr("value","");$("#experience-link").attr("value","");$("#experience-item-list").html("");experienceEdit.items[a]={type:"link",name:b,link:c};repopulateExperienceItems()}
...
resume.experienceInfo[a].endDate)+"</h3>",d=0;d<b.length;d++)"link"==b[d].type?c+='<a href="'+b[d].link+'" target="_blank">'+b[d].name+"</a><br>":"desc"==b[d].type&&(c+="<p><strong>"+b[d].desc+"</strong></p>");c+="</div>";$("#experience-container").append(c);$("#experience-edit-"+a).click(function(){var a=$(this).attr("id").substring(16);editExperience(a)});$("#experience-delete-"+a).click(function(){var a=$(this).attr("id").substring(18);resume.experienceInfo.splice(a,1);revalidateExperienceField()})}}
...
function addLinkActivity(){var a=activityEdit.items.length,b=$("#activity-link-name").attr("value"),c=$("#activity-link").attr("value");0!=c.indexOf("http")&&(c="http://"+c);$("#activity-link-name").attr("value","");$("#activity-link").attr("value","");$("#activity-item-list").html("");activityEdit.items[a]={type:"link",name:b,link:c};repopulateActivityItems()}
...
function repopulateActivityItems(){$("#activity-item-list").html("");for(var a=0;a<activityEdit.items.length;a++){var b=activityEdit.items[a],b="desc"==b.type?"<p><strong>"+b.desc+"</strong></p>":'<a href="'+b.link+'" target="_blank">'+b.name+"</a>";$("#activity-item-list").append('<li class="span3 activity-item" id="activity-'+a+'"><button class="close pull-right" id="activity-'+a+'">x</button>'+b+"</li>");$("button#activity-"+a).click(function(){var a=$(this).attr("id").substring(9);$("li#activity-"+
...
"</h3>",d=0;d<b.length;d++)"link"==b[d].type?c+='<a href="'+b[d].link+'" target="_blank">'+b[d].name+"</a><br>":"desc"==b[d].type&&(c+="<p><strong>"+b[d].desc+"</strong></p>");c+="</div>";$("#activity-container").append(c);$("#activity-edit-"+a).click(function(){var a=$(this).attr("id").substring(16);editActivity(a)});$("#activity-delete-"+a).click(function(){var a=$(this).attr("id").substring(18);resume.activityInfo.splice(a,1);revalidateActivityField()})}}
function editActivity(a){alert(activityEdit);alert(resume.activityInfo.length);alert(resume.activityInfo[a]);activityEdit.isEdit=!0;activityEdit.index=a;activityEdit.items=resume.activityInfo[a].items;var b=resume.activityInfo[a].position,c=resume.activityInfo[a].startDate,d=resume.activityInfo[a].endDate,a=resume.activityInfo[a].group;$("#activity-position").attr("value",b);$("#activity-start-date").attr("value",c);$("#activity-end-date").attr("value",d);$("#activity-group").attr("value",a);repopulateActivityItems()}
...
var timerId=void 0;
$(document).ready(function(){$("input").keypress(function(){resetTimer()});$("select").change(function(){resetTimer()});$("#education-award-add").click(function(){addAward()});$("#education-save").click(function(){saveEducation();pushDraft()});$("#add-education-btn").click(function(){educationEdit.isEdit&&($("#skill-category").attr("value",""),$("#skill-list").html(""));educationEdit.isEdit=!1;educationEdit.awards=[]});$("#skill-add").click(function(){addSkill()});$("#skill-save").click(function(){saveSkillCategory();pushDraft()});
$("#add-skill-btn").click(function(){skillEdit.isEdit=!1;skillEdit.skills=[]});$("#experience-fact-add").click(function(){addDescExperience()});$("#experience-link-add").click(function(){addLinkExperience()});$("#experience-save").click(function(){saveExperienceCategory();pushDraft()});$("#btn-add-experience").click(function(){experienceEdit.isEdit=!1;experienceEdit.items=[]});$("#activity-fact-add").click(function(){addDescActivity()});$("#activity-link-add").click(function(){addLinkActivity()});
$("#activity-save").click(function(){saveActivityCategory();pushDraft()});$("#btn-add-activity").click(function(){activityEdit.isEdit=!1;activityEdit.items=[]});$("#code-preview").click(function(){pushDraft();$("#basic-resume").attr("value");$.post("../resumepreview/index.php",{content:JSON.stringify(resume)},function(a){window.open("","_blank").document.writeln(a)})});$("#code-submit").click(function(){completeResume();var a=$("#basic-resume").attr("value");$.post("../private/php_scripts/addResume.php",
{uid:uid,resume:JSON.stringify(resume),name:a},function(){clearDraft();window.location.href="../users/"+username})})});
...
function resetTimer(){void 0!=timerId&&clearTimeout(timerId);timerId=setTimeout("pushDraft()",1E3)}function pushDraft(){void 0!=timerId&&clearTimeout(timerId);completeResume();var a=$("#basic-resume").attr("value");$.post("../private/php_scripts/draft.php",{request:"push",uid:uid,content:JSON.stringify(resume),name:a},function(){})}function clearDraft(){$.post("../private/php_scripts/draft.php",{request:"clear",uid:uid},function(){})};